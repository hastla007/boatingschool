...
<x-superadmin-layout>
    <x-slot name="header">
        <div>
            ...
            <h2 class="font-semibold text-xl text-slate-800 dark:text-slate-200">Neuer Tipp</h2>
        </div>
    </x-slot>

    <div class="max-w-2xl bg-white dark:bg-slate-800 rounded-2xl shadow-sm p-6">
        <form method="POST" action="{{ route('superadmin.tips.store') }}" enctype="multipart/form-data" class="space-y-5">
            @csrf
